...
**🎯 Before We Start: You've Got This!**

If you're feeling nervous about coding, that's completely normal! Every expert developer started exactly where you are. Here's the secret: **we're going to build your confidence step by step**.

**What makes this different:**
- ✅ **No prior coding experience needed**
- ✅ **Every concept explained in plain English first**
- ✅ **Interactive practice with instant feedback**
- ✅ **Real business scenarios, not abstract examples**
- ✅ **Safety nets to prevent frustration**
...
**🔍 Before You See Code: Let's Think Like Humans First**

Imagine you're explaining to a friend how to calculate compound interest:
1. "Take the starting amount"
2. "Add the interest rate" 
3. "Multiply by itself for each year"
4. "Give me the final amount"

**Now let's see how we tell the computer the same thing:**

\`\`\`apex
public class FinancialCalculator {
    public static Decimal calculateCompoundInterest(
        Decimal principal,    // Starting amount
        Decimal rate,        // Interest rate  
        Integer years        // Number of years
    ) {
        // Do the math: principal * (1 + rate) ^ years
...
**🔍 Code Detective Work - Let's Decode This Together:**

**Line 1:** \`public class FinancialCalculator {\`
- **Think:** "I'm creating a toolbox called FinancialCalculator"
- **"public"** = Anyone can use this toolbox
- **"class"** = It's a container for related tools

**Line 2:** \`public static Decimal calculateCompoundInterest(\`
- **Think:** "I'm creating a specific tool inside the toolbox"
- **"public static"** = Anyone can use this tool directly
- **"Decimal"** = This tool will give back a precise number
- **"calculateCompoundInterest"** = The name of our tool

**Lines 3-5:** The ingredients our tool needs
- **"Decimal principal"** = Starting money amount
- **"Decimal rate"** = Interest rate (like 0.05 for 5%)
- **"Integer years"** = How many years

**Line 7:** The actual calculation
- **"Math.pow(1 + rate, years)"** = Computer's way of doing (1 + rate) × (1 + rate) × ... for each year
- **"principal *"** = Multiply starting amount by the result

**Line 8:** Give back the answer
- **"return result"** = Hand the final number back to whoever asked

**🎉 Congratulations! You just read your first real Salesforce code!**
...
Welcome to the world of automated business logic!

**🎯 Before We Dive In: You're Ready for This!**

If you've made it this far, you already understand Apex basics. Now we're going to teach you something that will make you feel like a true automation wizard: **Triggers**!

**What makes this different:**
- ✅ **Real-world scenarios** - Every example solves actual business problems
- ✅ **Step-by-step breakdowns** - We'll decode every line together
- ✅ **Safety first** - Learn best practices to avoid common pitfalls
- ✅ **Confidence building** - Start simple, build complexity gradually

Ready to make Salesforce automatically respond to data changes? Triggers are your gateway to building intelligent, responsive systems that work 24/7 without human intervention.

## What Are Triggers? (Think of Them as Smart Watchers) 👀

**🔍 Before You See Code: Let's Think Like Humans First**

Imagine you're a security guard at a building:
1. **Someone enters** → You check their ID and log their visit
2. **Someone leaves** → You update the visitor log
3. **Someone tries to enter restricted area** → You stop them and alert management
4. **Fire alarm goes off** → You automatically lock down certain areas

**Triggers work exactly the same way** - they're like smart security guards for your data:
- **When data changes** → Trigger "watches" and can take action
- **Before it's saved** → Can validate, modify, or stop the change
- **After it's saved** → Can update related records or send notifications

**Real Business Example:**
- When a new Account is created → Automatically create a Welcome Task for the sales rep
- When an Opportunity closes → Update the Account's "Last Sale Date" 
- When a Contact email changes → Verify the new email format is valid

**🚀 Why This Matters:** Triggers let you build complex business logic that runs automatically. Master triggers, and you become the developer who can solve any business requirement!

## Your First Trigger: Step-by-Step Breakdown 🔍

Let's build a simple trigger together. **Don't worry about memorizing everything** - focus on understanding the pattern!

**Business Scenario:** Every time someone creates a new Account, we want to automatically create a Welcome Task for the sales rep.

\`\`\`apex
trigger AccountTrigger on Account (after insert) {
    // Step 1: Create a list to hold our new tasks
    List<Task> tasksToCreate = new List<Task>();
    
    // Step 2: Loop through each new account
    for (Account newAccount : Trigger.new) {
        // Step 3: Create a welcome task for each account
        Task welcomeTask = new Task();
        welcomeTask.Subject = 'Welcome new client: ' + newAccount.Name;
        welcomeTask.WhatId = newAccount.Id;  // Link to the account
    ...
    // Step 5: Save all tasks at once (efficient!)
...
1. **\`trigger AccountTrigger on Account (after insert)\`**
   - "Hey Salesforce, watch the Account object"
   - "Run this code AFTER new accounts are inserted"

2. **\`List<Task> tasksToCreate = new List<Task>();\`**
   - "Create an empty shopping cart for tasks"
   - We'll add tasks to this cart, then save them all at once

3. **\`for (Account newAccount : Trigger.new)\`**
   - "For each new account that was just created..."
   - \`Trigger.new\` is a special list containing all new records

4. **\`Task welcomeTask = new Task();\`**
   - "Create a new task (but don't save it yet)"

5. **\`welcomeTask.Subject = 'Welcome new client: ' + newAccount.Name;\`**
   - "Set the task subject to include the account name"

6. **\`insert tasksToCreate;\`**
   - "Save all tasks to the database at once"
   - This is more efficient than saving one at a time!

## Trigger Context Variables: Your New Best Friends 🤝

Salesforce gives you special variables to work with:

**\`Trigger.new\`** - List of new/updated records
**\`Trigger.old\`** - List of records before changes
**\`Trigger.newMap\`** - Map of new records (Id → Record)
**\`Trigger.oldMap\`** - Map of old records (Id → Record)

**When to use each:**
- **Before Insert:** Use \`Trigger.new\` to validate or modify data
- **After Insert:** Use \`Trigger.new\` to create related records
- **Before Update:** Compare \`Trigger.new\` vs \`Trigger.old\` to see what changed
- **After Update:** Use both to update related records based on changes

## Best Practices: Build Like a Pro 🌟
...
Keep your trigger clean and move logic to a handler:
...
public class AccountTriggerHandler {
    public static void handle() {
        ...
        if (Trigger.isAfter && Trigger.isInsert) {
            createWelcomeTasks(Trigger.new);
        }
    }
}
\`\`\`
...
### 3. Bulkify Your Code
Always assume you're processing multiple records:

**❌ This breaks with bulk operations:**
\`\`\`apex
for (Account acc : Trigger.new) {
    Task t = new Task(Subject = 'Welcome', WhatId = acc.Id);
    insert t;  // DON'T insert inside a loop!
}
\`\`\`
...
for (Account acc : Trigger.new) {
    tasks.add(new Task(Subject = 'Welcome', WhatId = acc.Id));
}
insert tasks;  // Insert all at once
\`\`\`

## Interactive Practice: Build Your Confidence! 💪

**Challenge 1: Contact Email Validation**
Create a trigger that prevents saving contacts with invalid email formats.

**Hint:** Use \`before insert, before update\` and check if email contains '@'

**Challenge 2: Opportunity Stage History**
When an Opportunity stage changes, create a Task to log the change.

**Hint:** Compare \`Trigger.new\` vs \`Trigger.oldMap\` to detect changes

## Safety Nets: Avoid Common Pitfalls ⚠️

### 1. Governor Limits
- **DML Statements:** Max 150 per transaction
- **SOQL Queries:** Max 100 per transaction
- **Solution:** Bulkify your code!

### 2. Recursive Triggers
Triggers can call themselves infinitely. Use static variables to prevent this:
...
public class AccountTriggerTest {
    ...
    static void testWelcomeTaskCreation() {
        ...
        List<Task> tasks = [SELECT Id FROM Task WHERE WhatId = :testAccount.Id];
        System.assertEquals(1, tasks.size(), 'Welcome task should be created');
    }
}
\`\`\`
...
### Example 1: Auto-assign Account Owner
\`\`\`apex
trigger AccountTrigger on Account (before insert) {
    for (Account acc : Trigger.new) {
        if (acc.OwnerId == null) {
            // Assign to default sales rep
            acc.OwnerId = '005000000000000';  // Replace with actual User ID
        }
    }
}
\`\`\`

### Example 2: Update Parent Account
\`\`\`apex
trigger ContactTrigger on Contact (after insert, after update) {
    ...
    for (Id accId : accountIds) {
        accountsToUpdate.add(new Account(
            Id = accId,
            Last_Contact_Update__c = System.now()
        ));
    }
...
**Coming up next:** We'll dive into Lightning Web Components and learn how to build modern, interactive user interfaces!

**💡 Pro Tip:** Practice these concepts in a Developer Org. The more you experiment, the more confident you'll become!`
            }
          }
        ],
        quiz: {
          id: 'apex-triggers-quiz',
          title: 'Apex Triggers Quiz',
          questions: [
            {
              id: 'trigger-basics',
              type: 'multiple-choice',
              question: 'When do BEFORE triggers execute?',
              options: [
                'After the record is saved to the database',
                'Before the record is saved to the database',
                'Only when manually called',
                'During batch operations only'
              ],
              correctAnswer: 'Before the record is saved to the database',
              explanation: 'BEFORE triggers execute before the record is saved, allowing you to validate or modify the data.',
              points: 20,
              difficulty: 'medium',
              category: 'triggers'
            }
        ...
        lessons: [
          {
            ...
            order: 1,
            content: {
              text: `# Lightning Web Components: Build Beautiful User Interfaces! ⚡

Welcome to the future of Salesforce development!

**🎯 Before We Dive In: You're Already Prepared!**

If you've made it this far, you understand Apex and data. Now we're going to teach you how to build beautiful, interactive user interfaces that your users will love! Think of this as learning to paint after you've learned to mix colors.

**What makes this different:**
- ✅ **Visual learning** - See every change instantly in your browser
- ✅ **Modern web standards** - Skills you can use anywhere
- ✅ **Step-by-step building** - From simple to sophisticated
- ✅ **Real user interfaces** - Build components users actually need

Ready to create interfaces that make complex data simple and beautiful? Let's build something amazing together!

## What Are Lightning Web Components? (Think of Them as Smart Building Blocks) 🧱

**🔍 Before You See Code: Let's Think Like Humans First**

Imagine you're building a house:
1. **Foundation** → HTML (the structure)
2. **Walls and rooms** → CSS (the appearance)
3. **Electricity and plumbing** → JavaScript (the functionality)
4. **Smart home system** → Lightning Platform (the integration)

**Lightning Web Components work exactly the same way** - they're smart building blocks for user interfaces:
- **HTML Template** → What users see (buttons, forms, lists)
- **CSS Styles** → How it looks (colors, fonts, layout)
- **JavaScript Controller** → What it does (handle clicks, load data)
- **Salesforce Integration** → Connect to your org's data automatically

**Real Business Example:**
- **Account Card Component** → Shows account info in a beautiful card
- **Contact List Component** → Displays contacts in an organized table
- **Opportunity Dashboard** → Interactive charts and metrics

**🚀 Why This Matters:** LWC lets you build modern, fast, beautiful interfaces that work on desktop and mobile. Master this, and you become the developer who can make any data look amazing!

## Your First Component: Step-by-Step Breakdown 🔍

Let's build a simple "Hello World" component together. **Don't worry about memorizing everything** - focus on understanding the pattern!

**Business Scenario:** Create a welcome message component that displays the current user's name.

### Step 1: The HTML Template (What Users See)
\`\`\`html
<!-- helloWorld.html -->
<template>
    <div class="welcome-container">
        <h1>Welcome to Salesforce, {userName}!</h1>
...
**🔍 Let's Decode This:**
- **\`<template>\`** → "This is what users will see"
- **\`{userName}\`** → "Show the user's name here" (dynamic data)
- **\`{currentDate}\`** → "Show today's date here" (dynamic data)
- **\`onclick={handleClick}\`** → "When clicked, run the handleClick function"

### Step 2: The JavaScript Controller (The Brain)
\`\`\`javascript
// helloWorld.js
import { LightningElement } from 'lwc';

export default class HelloWorld extends LightningElement {
    // Properties (data that can change)
    userName = 'Amazing Developer';
    currentDate = new Date().toLocaleDateString();
    
    // Methods (functions that do things)
    handleClick() {
        // This runs when the button is clicked
        alert('Hello from Lightning Web Components!');
    }
}
\`\`\`
...
1. **\`import { LightningElement } from 'lwc';\`**
   - "Import the Lightning Web Component superpowers"

2. **\`export default class HelloWorld extends LightningElement\`**
   - "Create a new component called HelloWorld with Lightning powers"

3. **\`userName = 'Amazing Developer';\`**
   - "Store the user's name in a variable"
   - This automatically shows up in the HTML where we wrote \`{userName}\`

4. **\`handleClick() { ... }\`**
   - "Define what happens when the button is clicked"

### Step 3: The CSS Styles (Make It Beautiful)
\`\`\`css
/* helloWorld.css */
.welcome-container {
    padding: 20px;
...
- Makes the text white and centered
- Styles the button to be interactive
- Adds hover effects for better user experience

### Step 4: The Configuration (Tell Salesforce About It)
\`\`\`xml
...
**🔍 What This Means:**
- **\`isExposed="true"\`** → "Make this component available to use"
- **\`targets\`** → "This component can be used on App Pages and Home Pages"

## Component Communication: Making Components Talk 🗣️

### Parent to Child (@api Properties)
**Think of this like giving instructions to a helper:**

**Parent Component:**
\`\`\`html
<template>
    <c-account-card record-id={selectedAccountId}></c-account-card>
</template>
\`\`\`

**Child Component (Account Card):**
\`\`\`javascript
import { LightningElement, api } from 'lwc';

export default class AccountCard extends LightningElement {
    @api recordId;  // Receives the account ID from parent
    
    // This automatically updates when recordId changes!
}
\`\`\`

**🔍 Plain English:** "Parent says: 'Hey Account Card, show me details for account ABC123.' Child responds: 'Got it, loading account ABC123 now!'"

### Child to Parent (Custom Events)
**Think of this like a child reporting back to a parent:**
...
handleAccountSelect() {
    ...
    const selectEvent = new CustomEvent('accountselect', {
        detail: { accountId: this.selectedAccount.Id }
    });
...
\`\`\`html
<template>
    <c-account-list onaccountselect={handleAccountSelection}></c-account-list>
</template>
\`\`\`
...
handleAccountSelection(event) {
    ...
    console.log('User selected account:', accountId);
}
\`\`\`

**🔍 Plain English:** "Child says: 'Hey parent, user clicked on account XYZ!' Parent responds: 'Thanks for letting me know, I'll update the dashboard.'"

## Working with Salesforce Data 📊

### Getting Data with Wire Service
\`\`\`javascript
...
**🔍 What This Does:**
1. **\`@wire(getAccounts)\`** → "Automatically call this Apex method"
2. **\`wiredAccounts({ error, data })\`** → "Handle the response"
3. **If successful** → Store data and clear any errors
4. **If failed** → Store error and clear data

### Displaying Data in Template
\`\`\`html
<template>
    <div if:true={accounts}>
        <div for:each={accounts} for:item="account" key={account.Id}>
    ...
    <div if:true={error}>
        <p>Error loading accounts: {error.body.message}</p>
    </div>
</template>
\`\`\`

**🔍 Plain English:**
- **\`if:true={accounts}\`** → "Only show this if we have accounts"
- **\`for:each={accounts}\`** → "Repeat this section for each account"
- **\`key={account.Id}\`** → "Use the account ID as a unique identifier"

## Interactive Practice: Build Your Confidence! 💪

**Challenge 1: Contact Card Component**
Create a component that displays contact information in a beautiful card format.

**Requirements:**
- Show contact name, email, and phone
- Add a "Call" button that shows an alert
- Style it with a nice border and shadow

**Challenge 2: Account Search Component**
Build a component with a search box that filters accounts as you type.

**Hint:** Use \`@track\` for the search term and filter the accounts array
...
**❌ Don't do this:**
\`\`\`html
<div for:each={accounts} for:item="account">
    <!-- This breaks if accounts is undefined -->
</div>
\`\`\`
...
<div if:false={accounts}>
    <p>Loading accounts...</p>
</div>
\`\`\`

### 2. Use Reactive Properties Correctly
**❌ This won't update the UI:**
\`\`\`javascript
this.accounts.push(newAccount);  // UI won't refresh
\`\`\`

**✅ This will update the UI:**
\`\`\`javascript
this.accounts = [...this.accounts, newAccount];  // UI refreshes
\`\`\`
...
### Example 1: Interactive Dashboard Card
\`\`\`javascript
// dashboardCard.js
import { LightningElement, api, wire } from 'lwc';
import getOpportunityStats from '@salesforce/apex/OpportunityController.getStats';

export default class DashboardCard extends LightningElement {
    @api title = 'Opportunities';
    ...
    get formattedTotal() {
        return this.stats ? \`$\${this.stats.total.toLocaleString()}\` : '$0';
    }
}
\`\`\`

### Example 2: Dynamic Form Component
\`\`\`javascript
// dynamicForm.js
import { LightningElement, api } from 'lwc';
...
**You've just learned:**
- ✅ What Lightning Web Components are and how they work
- ✅ How to build your first component step-by-step
- ✅ How to make components communicate with each other
- ✅ How to work with Salesforce data safely
- ✅ Best practices for professional development
- ✅ Real-world examples you can use immediately

**Coming up next:** We'll dive into Testing & Debugging and learn how to ensure your code works perfectly every time!

**💡 Pro Tip:** Start with simple components and gradually add complexity. Every expert was once a beginner who kept practicing!`,
              interactive: [
                {
                  type: 'code-editor',
                  data: {
                    title: 'LWC Interactive Code Editor',
                    description: 'Practice building Lightning Web Components',
                    codeExamples: [
          ...
          questions: [
            {
              id: 'lwc-q1',
              type: 'multiple-choice',
              question: 'What decorator is used to make a property public in LWC?',
              options: ['@track', '@api', '@wire', '@public'],
              correctAnswer: '@api',
              explanation: '@api decorator makes a property public and allows it to be set by parent components.',
              points: 20,
              difficulty: 'medium',
              category: 'lwc'
            }
...
Don't worry if "testing" sounds intimidating - by the end of this lesson, you'll understand why testing is actually your best friend as a developer, and you'll have the confidence to write tests that make your code rock-solid.

## Think of Testing Like This... 🏗️

Imagine you're building a house. Would you:
- Build it and hope it doesn't fall down? 
- OR test each beam, check each foundation, and make sure everything works perfectly?

That's exactly what testing does for your code! It's like having a quality inspector who checks every piece of your code to make sure it works perfectly, every single time.

## Why Testing Matters (And Why You'll Love It!) 🎯

**Here's the truth:** Testing isn't extra work - it's what separates professional developers from beginners. Here's why:

### 1. **Confidence Boost** 💪
- Your code works? You KNOW it works (not just hope)
- Making changes? No fear - your tests will catch any problems
- Deploying to production? Sleep peacefully knowing everything is tested

### 2. **Salesforce Requirement** ✅
- Salesforce requires 75% code coverage for production deployments
- But we're aiming for 90%+ because we're building quality code!

### 3. **Future-Proof Your Code** 🛡️
- Someone changes your code later? Tests will catch if they break something
- You forget how your code works in 6 months? Tests document exactly what it should do

## Your First Test: Baby Steps 👶

Let's start with something super simple. Here's a basic Apex class:
...
public class CalculatorTest {
    ...
    @IsTest
    static void testAdd() {
        // Step 1: Set up our test data
        Integer firstNumber = 5;
        Integer secondNumber = 3;
        
        // Step 2: Call the method we're testing
        Integer result = Calculator.add(firstNumber, secondNumber);
        
        // Step 3: Check if we got the right answer
        System.assertEquals(8, result, 'Adding 5 + 3 should equal 8');
    }
}
\`\`\`
...
1. **@IsTest** - This tells Salesforce "Hey, this is a test class!"
2. **@IsTest static void testAdd()** - This is our test method
3. **Set up data** - We prepare what we need for the test
4. **Call the method** - We actually run the code we're testing
5. **Assert the result** - We check if we got what we expected

**That's it!** You just wrote your first test! 🎉

## The Magic Formula: AAA Pattern 📝

Every good test follows this pattern:

### **A**rrange 📋
Set up your test data and conditions
\`\`\`apex
...
Contact newContact = ContactService.createContact(testAccount.Id, 'John', 'Doe');
\`\`\`

### **A**ssert ✅
Verify the results are what you expected
\`\`\`apex
// Assert: Check if everything worked correctly
System.assertNotEquals(null, newContact.Id, 'Contact should be created');
System.assertEquals('John', newContact.FirstName, 'First name should be John');
System.assertEquals(testAccount.Id, newContact.AccountId, 'Contact should be linked to account');
\`\`\`

## Real-World Example: Testing a Contact Creator 🌍

Let's test something more realistic. Here's a service class that creates contacts:
...
**Now let's write a comprehensive test:**
...
public class ContactServiceTest {
    ...
    static void testCreateContact() {
        ...
        // ACT: Call the method we're testing
        Test.startTest(); // This starts our test context
        Contact result = ContactService.createContact(testAccount.Id, firstName, lastName);
        Test.stopTest(); // This ends our test context
        
        // ASSERT: Verify everything worked correctly
        System.assertNotEquals(null, result.Id, 'Contact should be created with an ID');
        System.assertEquals(firstName, result.FirstName, 'First name should match');
        System.assertEquals(lastName, result.LastName, 'Last name should match');
        System.assertEquals(testAccount.Id, result.AccountId, 'Contact should be linked to account');
        System.assertEquals('jane.smith@company.com', result.Email, 'Email should be auto-generated');
        
        // Double-check by querying the database
        Contact savedContact = [SELECT Id, FirstName, LastName, Email, AccountId 
                               FROM Contact 
                               WHERE Id = :result.Id];
        System.assertEquals(firstName, savedContact.FirstName, 'Contact should be saved correctly');
    }
}
\`\`\`

## Testing Best Practices (Your Success Toolkit) 🛠️
...
- ❌ \`testMethod1()\`
- ✅ \`testCreateContact_WithValidData_ShouldCreateContactSuccessfully()\`

### 3. **Test Both Success AND Failure**
\`\`\`apex
@IsTest
static void testCreateContact_WithNullAccountId_ShouldThrowException() {
    // Test what happens when something goes wrong
    try {
        ContactService.createContact(null, 'John', 'Doe');
...
Create a helper class to generate test data:
...
## Common Testing Mistakes (And How to Avoid Them) ⚠️

### ❌ **Mistake 1:** Not using Test.startTest() and Test.stopTest()
**Why it matters:** These methods reset governor limits and ensure proper test isolation.

### ❌ **Mistake 2:** Testing in the same transaction
**Fix:** Always use separate transactions for setup and testing.

### ❌ **Mistake 3:** Not testing edge cases
**Remember:** Test with null values, empty strings, large datasets, etc.

### ❌ **Mistake 4:** Hard-coding IDs
**Fix:** Always create test data dynamically.

## Your Testing Confidence Builder 💪

**Remember these truths:**
- Every expert developer started exactly where you are now
- Writing tests gets easier with practice (like riding a bike!)
- Good tests save you HOURS of debugging later
- Tests are documentation that never goes out of date

## Practice Challenge 🎯

Try writing a test for this simple class:
...
**Your mission:**
1. Create a test class called \`StringHelperTest\`
2. Test with a normal string like "hello"
3. Test with an empty string
4. Test with a null value
5. Test with a single character

## What You've Mastered 🏆

By completing this lesson, you now understand:
- ✅ Why testing is your best friend as a developer
- ✅ The AAA pattern (Arrange, Act, Assert)
- ✅ How to write your first test methods
- ✅ Best practices for professional testing
- ✅ How to avoid common testing mistakes
- ✅ Real-world testing examples you can use immediately

**Coming up next:** We'll dive into advanced testing techniques including bulk data testing, mock objects, and integration testing!

**💡 Pro Tip:** Start small, test often, and remember - every line of tested code is a line of code you can trust completely!`
            }
          }
        ],
        quiz: {
          id: 'testing-quiz',
          title: 'Apex Testing Mastery Quiz',
          questions: [
            {
              id: 'test-q1',
              type: 'multiple-choice',
              question: 'What annotation is used to mark a class as a test class?',
              options: ['@Test', '@IsTest', '@TestClass', '@UnitTest'],
              correctAnswer: '@IsTest',
              explanation: '@IsTest annotation marks a class or method as a test in Salesforce.',
              points: 20,
              difficulty: 'easy',
              category: 'testing'
            },
            {
              ...
              options: ['Act, Assert, Arrange', 'Arrange, Act, Assert', 'Assert, Act, Arrange', 'Arrange, Assert, Act'],
              correctAnswer: 'Arrange, Act, Assert',
              explanation: 'AAA pattern: Arrange (setup), Act (execute), Assert (verify) - the standard structure for test methods.',
              points: 30,
              difficulty: 'medium',
              category: 'testing'
            }
      ...
      },
      {
        ...
        description: 'Master REST API development and external system integration',
        duration: '2 weeks',
        completed: false,
        locked: true,
        order: 5,
        lessons: [
          {
            ...
            title: 'REST APIs: Connect Your Salesforce to the World',
            type: 'content',
            duration: '45 min',
            completed: false,
            points: 450,
            order: 1,
            content: {
              text: `# REST APIs: Your Gateway to the Digital Universe! 🌐

**Welcome to the world of connected systems!**

Don't worry if "REST API" sounds like technical jargon - by the end of this lesson, you'll understand how APIs work and feel confident building your own integrations that connect Salesforce to any system in the world.
...
That's exactly how APIs work! They're the "waiters" that help different systems talk to each other.

## What is REST? (Keep It Simple!) 🛣️

**REST** stands for "Representational State Transfer" - but forget the fancy name. Here's what you need to know:

### REST is like a set of rules for how systems should talk:
1. **GET** - "Please give me some data" (like asking for the menu)
2. **POST** - "Please create something new" (like placing an order)
3. **PUT** - "Please update this completely" (like changing your entire order)
4. **PATCH** - "Please update just this part" (like adding extra cheese)
5. **DELETE** - "Please remove this" (like canceling an item)

## Your First Salesforce REST API 🚀

Let's build something real! Here's a simple REST service that manages contacts:
...
### 1. **@RestResource** - The Front Door 🚪
\`\`\`apex
@RestResource(urlMapping='/api/contacts/*')
\`\`\`
This tells Salesforce: "Hey, when someone visits /api/contacts/, send them to this class!"

### 2. **@HttpGet** - Reading Data 📖
\`\`\`apex
@HttpGet
global static Contact getContact() {
\`\`\`
This method handles GET requests - when someone wants to READ contact data.

### 3. **@HttpPost** - Creating Data ✍️
\`\`\`apex
@HttpPost
global static String createContact(...) {
\`\`\`
This method handles POST requests - when someone wants to CREATE new contact data.

## Real-World Example: Weather Integration 🌤️

Let's build something practical - getting weather data for your accounts:

\`\`\`apex
public class WeatherService {
    
    // Method to get weather for an account's city
    public static String getWeatherForAccount(Id accountId) {
        // Get the account's billing city
        Account acc = [SELECT BillingCity FROM Account WHERE Id = :accountId];
        
        if (String.isBlank(acc.BillingCity)) {
            return 'No city found for this account';
        }
        
        // Call external weather API
        return callWeatherAPI(acc.BillingCity);
    }
    ...
    private static String callWeatherAPI(String city) {
        ...
        try {
            ...
            if (response.getStatusCode() == 200) {
                ...
                Decimal tempKelvin = (Decimal) main.get('temp');
                Decimal tempCelsius = tempKelvin - 273.15;
                
                return 'Current temperature in ' + city + ': ' + 
                       tempCelsius.setScale(1) + '°C';
            } else {
                return 'Weather service unavailable. Status: ' + response.getStatusCode();
            }
...
APIs speak JSON (JavaScript Object Notation). Here's how to work with it in Apex:
...
## Authentication: Your Digital Keys 🔐

Most APIs require authentication. Here are common patterns:
...
## Error Handling: Your Safety Net 🛡️

Always handle errors gracefully:

\`\`\`apex
public static String safeAPICall(String endpoint) {
    try {
        Http http = new Http();
        HttpRequest request = new HttpRequest();
        request.setEndpoint(endpoint);
...
### 1. **Use Remote Site Settings**
Before making any external calls, register the endpoint in Setup > Remote Site Settings.

### 2. **Handle Timeouts**
\`\`\`apex
request.setTimeout(10000); // 10 seconds max
\`\`\`

### 3. **Respect Rate Limits**
Many APIs limit how many calls you can make. Implement retry logic:

\`\`\`apex
public static String callWithRetry(String endpoint, Integer maxRetries) {
    for (Integer i = 0; i < maxRetries; i++) {
        HttpResponse response = makeCall(endpoint);
...
\`\`\`apex
@future(callout=true)
public static void processAccountsAsync(List<Id> accountIds) {
...
Build a simple integration that:
1. Takes a Contact ID
2. Calls a public API to get a random quote
3. Updates the Contact's Description with the quote

**Hint:** Use https://api.quotable.io/random for free quotes!

## What You've Mastered 🏆

By completing this lesson, you now understand:
- ✅ What REST APIs are and how they work
- ✅ How to build REST services in Salesforce
- ✅ How to make external API calls from Apex
- ✅ JSON parsing and creation
- ✅ Authentication patterns
- ✅ Error handling best practices
- ✅ Real-world integration examples

**Coming up next:** We'll dive into advanced integration patterns, webhooks, and building robust, scalable integrations!

**💡 Pro Tip:** Start with simple integrations and gradually add complexity. Every API integration follows the same basic pattern - you've got this!`
            }
          }
        ],
        quiz: {
          ...
          questions: [
            {
              ...
              correctAnswer: 'GET',
              explanation: 'GET method is used to retrieve/read data from a REST API without modifying it.',
              points: 20,
              difficulty: 'easy',
              category: 'rest-api'
            },
            {
              id: 'api-q2',
              type: 'multiple-choice',
              question: 'What annotation is used to create a REST resource in Salesforce?',
              options: ['@RestResource', '@HttpResource', '@ApiResource', '@WebService'],
              correctAnswer: '@RestResource',
              explanation: '@RestResource annotation is used to expose an Apex class as a REST web service.',
              points: 30,
              difficulty: 'medium',
              category: 'rest-api'
            }
...
export default simplifiedLearningPaths;
